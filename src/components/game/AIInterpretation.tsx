...
export const AIInterpretation: React.FC<AIInterpretationProps> = ({
  ...
  const exportSessionData = () => {
    try {
      ...
      link.href = url;
      link.download = `glass-bead-game-session-${sessionData.id?.slice(-8) || Date.now()}.json`;
  ...
  const generateConceptualInterpretation = (data: SessionData): string => {
    ...
    const interpretations = [
      ...
      This contemplative exercise can be understood as a living demonstration of how knowledge structures itself when freed from the constraints of linear thinking. The concepts did not merely coexist in space but actively influenced each other's positioning, creating what we might call a 'cognitive field' - a zone where ideas exert mutual influence across the three fundamental dimensions of truth, beauty, and goodness.`,

      `The session unfolded as an exploration of the hidden geometries that connect ${activeConcepts.slice(0, 2).join(' and ')}, revealing patterns analogous to ${selectedAnalogy}. Through ${interactions.length} distinct interactions, the contemplation traced ${selectedMovement}, suggesting that these concepts share an underlying structural affinity that becomes visible only when they are allowed to move freely through multidimensional space.
      ...
      This arrangement mirrors ${selectedAnalogy} in its capacity to generate emergent properties that cannot be predicted from the individual components alone. The contemplation created a temporary intellectual ecosystem where ideas evolved in real-time, adapting to their conceptual environment and forming new relationships based on their essential natures rather than their traditional academic classifications.

      The final configuration represents a stable yet dynamic equilibrium - a moment where the concepts achieved maximum mutual illumination while maintaining their individual integrity. This session demonstrates how the Glass Bead Game functions as both analytical tool and meditative practice, revealing the hidden architectures that connect human knowledge across all domains of understanding.`
    ];
      ...
      <div className="min-h-screen bg-gradient-to-br from-indigo-950 via-purple-900 to-black text-white flex items-center justify-center">
        <Card className="bg-gray-900 border-gray-700 p-8 max-w-2xl w-full text-center">
          ...
          <p className="text-gray-300">
            Interpreting the three-dimensional patterns and emergent meanings...
          </p>
        </Card>
      </div>
    );
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-center gap-4">
          ...
          <Button
            ...
            variant="outline"
            className="border-green-400 text-green-400 hover:bg-green-400 hover:text-white px-6 py-3"
          >
          ...
          <Button
            ...
            onClick={() => navigator.share && navigator.share({ 
              title: 'Glass Bead Game Contemplation', 
