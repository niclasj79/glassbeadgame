...
- Validation: local commands are documented in `docs/audits/M0-VALIDATION-COMMANDS.md`; protected `Quality Gates` passed for PR #19 and its exact `main` merge commit `3383f50`, then passed for PR #21 and its exact `main` merge commit `4c2175b` in CI run `29257407771`.
...
- Ready domain work: M1-002 was accepted in PR #21; M1-003 is fully specified, depends only on Done M1-001 and M1-002, owns an isolated domain replay boundary, and has objective acceptance criteria and checks.
...
Codex may select M1-003 as the first eligible Ready task after this queue-advancement change is reviewed and merged.
