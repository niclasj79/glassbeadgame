...
// Project 3D point to 2D canvas with zoom support
export const project3DTo2D = (x: number, y: number, z: number, canvas: HTMLCanvasElement, zoom: number = 1) => {
  const distance = 300 / zoom; // Closer camera when zoomed in
  const scale = distance / (distance + z);
  return {
    x: (x * scale * zoom) + canvas.width * 0.5,
    y: (y * scale * zoom) + canvas.height * 0.5,
    scale: scale * zoom
  };
};
...
const getTrigValues = (angle: number) => {
  const key = Math.round(angle * 1000).toString();
...
export const getTouchCenter = (touch1: { x: number; y: number }, touch2: { x: number; y: number }) => {
  return { x: (touch1.x + touch2.x) * 0.5, y: (touch1.y + touch2.y) * 0.5 };
};
...
export const screenToSphere = (screenX: number, screenY: number, canvas: HTMLCanvasElement, rotationRef: React.MutableRefObject<{ x: number; y: number }>) => {
  ...
  return { x: inverseRotated.x, y: inverseRotated.y, z: inverseRotated.z };
};
...
export const calculateDimensionalValues = (x: number, y: number, z: number) => {
  ...
  if (radiusSquared === 0) {
    return { abstract_concrete: 0.5, theoretical_practical: 0.5, analytical_intuitive: 0.5 };
  }
  const radius = Math.sqrt(radiusSquared);
  const radiusInverse = getRadiusInverse(radius);
  return {
    abstract_concrete: (z * radiusInverse + 1) * 0.5,
    theoretical_practical: (y * radiusInverse + 1) * 0.5,
    analytical_intuitive: (x * radiusInverse + 1) * 0.5,
  };
};
